...
static
int inj_gather_functions(lh_session_t *lh)
{
...
static
int inj_process (lh_session_t* lh)
{
        int rc = LH_SUCCESS;
        do {
                ...
                lh->exe_symbols = exe_load_symbols(filename, 
                                &lh->exe_symbols_num,
                                &lh->exe_entry_point,
                                &lh->exe_interp,
                                &lh->is64);
	        ...
	        LH_VERBOSE(1, "Executable headers are loaded");
                ...
                if (lh->exe_symbols && lh->exe_symbols_num > 0) {
                        qsort(lh->exe_symbols, lh->exe_symbols_num,
                                        sizeof(*lh->exe_symbols), elf_symbol_cmpqsort);
                }
   ...
   uintptr_t lhm_munmap) 
{
   ...
   for(i = 1; i <= 8192; i++)  // 4096*4096 => +-16 MBytes
   {
    ...
    if(address < 16384) continue; 

    uintptr_t wanted_address = (uintptr_t) address;
    LH_VERBOSE(4, "Wanted address for mmap: "LX, wanted_address);

    int rc;
    if((rc=inj_trap(lh->proc.pid, iregs))!= LH_SUCCESS) break;
    if((rc=inj_pass_args2func(iregs, lhm_mmap, wanted_address, MMAP_SIZE))!= LH_SUCCESS) break;
    if((rc = inj_setexecwaitget(lh, "lhm_mmap", iregs))!= LH_SUCCESS) break;
    ...
    if((rc=inj_trap(lh->proc.pid, iregs))!= LH_SUCCESS) break;
    if((rc=inj_pass_args2func(iregs, lhm_munmap, returned, MMAP_SIZE))!= LH_SUCCESS) break;
    if((rc = inj_setexecwaitget(lh, "lhm_munmap", iregs))!= LH_SUCCESS) break;
  }
        ...
        do {
	        ...
	        lh_rset_sp(&iregs, lh_rget_sp(&iregs) - lh_redzone());
                ...
                for (idx = 0; idx < sizeof(stack)/sizeof(uintptr_t); ++idx) {
                        if ((rc = inj_peekdata(lh->proc.pid, lh_rget_sp(&iregs) +
                                                        idx * sizeof(size_t), &stack[idx])) != LH_SUCCESS)
                                break;
                }
                if (rc < 0)
                        break;
                ...
                /* Call malloc */
                if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                if((rc=inj_pass_args2func(&iregs, lh->fn_malloc, tgtsz, 0))!= LH_SUCCESS) break;
                lh_dump_regs(&iregs);
                if((rc = inj_setexecwaitget(lh, "malloc", &iregs)) != LH_SUCCESS) break;
                lh_dump_regs(&iregs);
                ...
                /* Call dlopen */
                if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                if((rc=inj_pass_args2func(&iregs, lh->fn_dlopen, result, (RTLD_LAZY | RTLD_GLOBAL)))!= LH_SUCCESS) break;
                if((rc = inj_setexecwaitget(lh, "dlopen", &iregs))!= LH_SUCCESS) break;
			...
			break;
                    ...
                    LH_VERBOSE(1, "Hook settnigs found, v%d", hook_settings->version);
                    ...
                    if(hook_settings->autoinit_pre != NULL)
                    {
                       ...
                       if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                       if((rc=inj_pass_args2func(&iregs, (uintptr_t) hook_settings->autoinit_pre, heap+dllsz+1, 0))!= LH_SUCCESS) break;
                       if((rc = inj_setexecwaitget(lh, "autoinit_pre", &iregs))!= LH_SUCCESS) break;
                       result = lh_rget_ax(&iregs);
                    ...
                    // lets walk through the functions wanted to be patched
		    ...
		    LH_VERBOSE(4, "Function hooks: "LX" / "LX" (size "LX")", (uintptr_t)fnh, hookend, (int)size);
                    while((uintptr_t)fnh < hookend) {
                      ...
                      if(fnh->hook_kind == LHM_FN_HOOK_TRAILING) break;
                      ...
                      LH_VERBOSE(1, "Function hook libname: %s, symbol: %s, offset: "LX", opcode bytes to restore: %d", fnh->libname, fnh->symname, fnh->sym_offset, fnh->opcode_bytes_to_restore);
                      ...
                      if(fnh->hook_kind == LHM_FN_HOOK_BY_NAME) {
                        symbol= ld_find_address(lib_to_hook, fnh->symname, NULL);
                      }
                      else if(fnh->hook_kind == LHM_FN_HOOK_BY_OFFSET) {
                        symbol= lib_to_hook->addr_begin + fnh->sym_offset;
                      }
                     ...
                     if(lib_to_hook->mmap == 0) 
                      ...
                      // we prepare the new stubs in the mmaped memory first
                      
                       if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                       if((rc=inj_pass_args2func(&iregs, lhm_memcpy, lib_to_hook->mmap, symbol))!= LH_SUCCESS) break;
                       if((rc = inj_setexecwaitget(lh, "lhm_memcpy", &iregs))!= LH_SUCCESS) break;
                      ...
                      if(LH_SUCCESS != inj_build_rel_jump(l_new_payload, symbol+fnh->opcode_bytes_to_restore, r_new_payload_address)) {
                      ...
                      //ok, lets call the replacement function with an absolute call
                      ...
                      // we store the original address now, if wanted
                      ...
                      if(LH_SUCCESS != inj_build_rel_jump(l_new_payload_start, r_hook_abs_jump_address, symbol)) {
                      ...
                      // safely write back these bytes
                      if(LH_SUCCESS != inj_copydata(lh->proc.pid, symbol, l_new_payload_start, rel_jump_size)) {
                      ...
                      if(lh_verbose > 3) {
                         ...
                         if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                         if((rc=inj_pass_args2func(&iregs, lhm_hexdump, symbol, 0x10))!= LH_SUCCESS) break;
                         if((rc = inj_setexecwaitget(lh, "lhm_hexdump", &iregs))!= LH_SUCCESS) break;


                         LH_VERBOSE(4, "Dumping the corresponding payload area");
                         if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                         if((rc=inj_pass_args2func(&iregs, lhm_hexdump, r_payload_start, hook_area))!= LH_SUCCESS) break;
                         if((rc = inj_setexecwaitget(lh, "lhm_hexdump", &iregs))!= LH_SUCCESS) break;
                      }
                    ...
                    if((hook_settings->autoinit_post != NULL)&&(hook_successful))
                    {
                       ...
                       if((rc=inj_trap(lh->proc.pid, &iregs))!= LH_SUCCESS) break;
                       if((rc=inj_pass_args2func(&iregs, (uintptr_t) hook_settings->autoinit_post, heap+dllsz+1, 0))!= LH_SUCCESS) break;
                       if((rc = inj_setexecwaitget(lh, "autoinit_post", &iregs))!= LH_SUCCESS) break;
                    }
